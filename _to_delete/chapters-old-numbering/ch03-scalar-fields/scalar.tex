...
Quantum field theory concerns the generalisation of quantum mechanics to relativistic settings in which particle number is not conserved. Many of its complexities arise from the infinities that occur in representing an infinite system with field degrees of freedom existing at every point in spacetime. Constructing field theories on lattice geometries imposes both ultraviolet (UV) and infrared (IR) regulators: the distance between lattice sites defines a shortest distance (highest energy) scale that is representable, while boundary conditions on a finite lattice limit arbitrarily long-wavelength (vanishingly small  energy) modes. This approach yields a system with a finite number of degrees of freedom and for the case of a scalar field it corresponds to a set of coupled oscillators at every point in the lattice. As with the single oscillator, the Hilbert space of the theory is infinite dimensional and the lattice theory structurally corresponds to a relativistic many body problem. The subtleties of continuum quantum field theory are pushed to the process of taking the continuum and infinite volume limits.

In this chapter, we will introduce scalar fields defined on a lattice geometry and examine their properties.   
...
\end{equation}
  ...
  \item Define a $Z^d$ spatial lattice of points separated by a distance $a$ (the lattice spacing\aidx{lattice spacing})
\begin{equation}
\begin{aligned}
...
\end{equation}
  ...
  \item Assume spatially periodic boundary conditions\aidx{Schrödinger functional!boundary conditions}\aidx{boundary conditions!periodic}: identify \(n_{i}=N\) with \(n_{i}=0\)
...
\end{equation}
  ...
  \item Now let us consider imaginary time\aidx{imaginary time} evolution as before

\begin{equation}
\exp (-\tau \hat{H})=\lim _{N_{\tau} \rightarrow \infty}(\hat{T}(\varepsilon))^{N_{T}} \quad \varepsilon=\frac{\tau}{N_{T}}
\end{equation}

with the transfer matrix\aidx{transfer operator} operator\aidx{transfer matrix}
...
which depends on the fields on timeslice \(s\) and \(s+1\)
  ...
  \item We can use this to construct the partition function\aidx{partition function} or correlation functions\aidx{correlation function} (with $\tau=\varepsilon N_{\tau}$, $t=\varepsilon N_{t}$)
...
\end{equation}
  ...
  \item With a small change from the central difference\aidx{finite difference!central} in space that arose from canonical quantization to a forward difference\aidx{finite difference!forward}, we can make space and time symmetric
...
where \(\varphi\left(n_{\mu}\right)=\varphi_{n_{4}}(\vec{n})\). Note that we could also make all finite differences central differences, but that breaks reflection positivity\aidx{reflection positivity}.
...
This was a mathematical convenience but has a close analogy with quantum statistical mechanics\aidx{statistical mechanics, relation to} at non-zero temperature\aidx{temperature}.
  \item If \(\beta=(k T)^{-1}\) (with \(k=\) Boltzmann's constant) then the partition function\aidx{partition function}
...
for the physical temperature \(T\) with \(\varphi(x+\beta \hat{t})=\varphi(x)\)
  ...
  \item For a lattice geometry with temporal extent \(N_{t}, \beta=a_{t} N_{t} \Rightarrow T=\frac{1}{k a_{t} N_{t}}\), where $a_t$ is the temporal lattice spacing\aidx{lattice spacing}
  \item The temperature can be varied by changing the \# of sites in the temporal direction at fixed lattice spacing \(a_{t}\)
  \item Alternatively one con consider an anisotropic discretisation\aidx{lattice!anisotropic} which allows the spatial lattice spacing to be held fixed while the temporal extent is charges continuously for a fixed number of sites
  \item For zero-temperature physics, in which $N_t\to\infty$, any boundary conditions\aidx{Schrödinger functional!boundary conditions} can be chosen and lead to effects that are suppressed as $e^{-N_t}$. For example, Dirichelet boundary conditions\aidx{boundary conditions!Dirichlet} are sometimes used in gauge theories as they allow gauge field topology to change 
...
\begin{itemize}
  \item Consider the action above \(\quad S_{E}[\varphi]=a^{d+1} \sum_{n_{\mu}\in\Lambda}\left\{\frac{1}{2 a^{2}} \sum_{v=1}^{d+1}\left[\varphi\left(n_{\mu}+\hat{\nu}\right)-\varphi\left(n_{\mu}\right)\right]^{2}+\frac{m^{2}}{2} \varphi\left(n_{\mu}\right)^{2}+V\left(\varphi\left(n_{\mu}\right)\right)\right\}\) with \(V=0\) in periodic boundary conditions\aidx{Schrödinger functional!boundary conditions}\aidx{boundary conditions!periodic}
  \item Can rewrite in terms of a Klein-Gordon operator\aidx{Klein-Gordon operator}
...
\end{equation}
  ...
  \item Defining the partition function\aidx{partition function} with external source \(J\)
...
\includegraphics[width=0.4\textwidth, center]{figures/fig-redraw-002.pdf}
...
The transfer matrix\aidx{transfer operator} introduced above defines the time evolution of time-sliced fields
...
and the Euclidean action\aidx{action!Euclidean} (Eq.~\eqref{eq:scalar_euclidean_action}) can be separated as
...
\begin{itemize}
  \item The transfer matrix defines the lattice Hamiltonian\aidx{Hamiltonian!lattice} through its matrix element or formally at an operator level
\end{itemize}
...
\begin{itemize}
  \item In order for a Euclidean QFT to have an analytic continuation to a well-defined Minkowski-space QFT, its Green functions (referred to as Schwinger functions\aidx{Schwinger functions}) must satisfy various requirements (Osterwalder-Schrader axioms\aidx{Osterwalder-Schrader axioms}). 
  \begin{enumerate}
      ...
      \item Cluster property\aidx{cluster property} ($\underset{\lambda\to\infty}{\lim}\langle {\cal O}(\lambda a)\tilde{\cal O}(0)\rangle-\langle{\cal O}(\lambda a)\rangle\langle\tilde{\cal O}(0)\rangle\to0$)
  \end{enumerate}
  Most of these are analogous to the Wightman axioms\aidx{Wightman axioms} of Minkowski field theory
...
[NB: for scalar theories, OS reconstruction thm\aidx{Osterwalder-Schrader reconstruction theorem} details how the Minkowski space theory can be built]
  \item Euclidean time reflection\aidx{reflection positivity} is
\end{itemize}
...
\end{equation}
or as a path integral\aidx{path integral} 
\begin{equation}
...
\item [(B)] Shift the same \(F\) by $n$ temporal lattice spacing\aidx{lattice spacing}s in the positive direction to define \(F^{\prime}\) and \(\left|\Psi^{\prime}\right\rangle=F^{\prime}|\Omega\rangle\) and now \(\Theta F^{\prime}\) is separated from \(F^{\prime}\) by \(2 n\) temporal sites\\
...
Thus \(\hat{T}^{2 n}=e^{-2 n a \hat{H}}\) must be a positive operator\aidx{transfer matrix} and hence eigenvalues of $\hat{H}$ are real.

\end{enumerate}

  \item Reflection positivity of all correlation functions\aidx{correlation function} ensures a Hilbert space and well-defined Hamiltonian (Hermitian in its Minkowski form) exists
...
and we see that the positivity criterion is met.
  ...
  \item e.g. staggered fermions\aidx{fermions!staggered} (later) are site reflection positive\aidx{reflection positivity!site reflection} not link reflection positive\aidx{reflection positivity!link reflection} so \(\hat{T}^{2}\) is positive \& real \& bounded but \(\hat{T}\) is not but still $\hat{T}^2 = e^{- 2 a \hat{H}}$ defines a Hermitian Hamiltonian after analytic continuation
...
\begin{itemize}
  \item Let is specify to a quartic potential\aidx{quartic potential} \(\quad V=\frac{m_0^{2}}{2} \varphi^{T} \varphi+\frac{\lambda_0}{4}\left(\varphi^{T} \varphi\right)^{2}\) which is only bounded from below if \(\lambda_{0} \geqslant 0\)
  \item If \(m_{0}^{2} \geqslant 0\), ground state has \(\langle\varphi\rangle=0\) and retains \(O(n)\) symmetry
  \item If \(m_{0}^{2}<0\), potential is minimized at \(\varphi_{g}^{T} \varphi_{g}=\left|\varphi_{g}\right|^{2}=v^{2}=-m_{0}^{2} / \lambda_{0}>0\) and in infinite volume for $d \geq 2$ spatial dimensions the vacuum spontaneously breaks\aidx{chiral symmetry!spontaneous breaking}\aidx{spontaneous symmetry breaking} the $O(n)$ symmetry [Mermin-Wagner\aidx{Mermin-Wagner theorem} \& Coleman Theorems\aidx{Coleman theorem} prohibit SSB in $d=0,1$] [no degenerate vacua in finite volume - Witten in QFT and Strings: a course for mathematicians]
...
\begin{itemize}
  \item Discretising the theory using forward finite difference\aidx{finite difference!forward}s \(\left(\partial_{\mu} \varphi_{n}^{a}=\left(\varphi_{n+\hat{\mu}}^{a}-\varphi_{n}^{a}\right) / a\right)\) gives the action
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
      \item for $n=1$, this is the $d+1$-dimensional Ising model\aidx{Ising model}
      \item for $n=3, d=2$ this is the $3d$ Heisenberg model\aidx{Heisenberg model}
  \end{itemize}
...
and by mean field arguments\aidx{mean field approximation} the vev\aidx{vacuum expectation value (vev)} can be shown to behave as

\begin{equation}
v^{2} \sim\left(\kappa-\kappa_{c}(\lambda)\right),
\end{equation}

vanishing smoothly as the critical line\aidx{critical line} is approached\\
...
\item The schematic phase diagram\aidx{phase diagram}\aidx{phase diagram!QCD} is shown in  Fig.~\ref{fig:On_phase}
\begin{figure}
    \centering
...
\end{figure}
  ...
  \item Importantly the transition line is a second order phase transition\aidx{phase transition, second-order} (order parameter\aidx{order parameter} \(v\) is continuous but its derivative is not)
  \item physical scales diverge when measured in lattice units as the critical line is approached and physics becomes independent of the discretisation - a continuum limit\aidx{continuum limit}
...
\begin{itemize}
  \item Just like in continuum QFT, it is possible (and sometimes useful) to perform a perturbative expansion of the lattice regulated path integral\aidx{path integral} (ie in the region shown in Fig.~\ref{fig:weak_phase}
  \begin{figure}
      \centering
      \redrawcompare{fig-notes-004.jpg}{fig-redraw-004.pdf}
      \caption{Region of phase diagram\aidx{phase diagram}\aidx{phase diagram!QCD} studied in the weak coupling expansion.
      \label{ig:weak_phase}}
  \end{figure}
  \item From what we saw with the single component field, the free propagator\aidx{scalar field!propagator} in the case of $O(n)$ is given by
\end{itemize}
...
\begin{itemize}
  \item Recall the vertex functions (one-particle irreducible $n$-point functions) are generated by the effective action\aidx{Schrödinger functional!coupling}\aidx{effective action}
\end{itemize}
...
This object is related to the partition function\aidx{partition function} $Z[J]$, which generates all diagrams, as $W[J]\equiv-\log Z[J]$ generates all connected diagrams and $\Gamma$ is then the Legendre transform of $W$ ($\Gamma[\bar\varphi]=-\int dx \bar\varphi(x)J(x)+W[J]$).
...
\begin{itemize}
  ...
  \item The connected green functions\aidx{Green functions!connected} are then given by (take functional derivative of \(W / \Gamma\) ) the following (and shown in Fig.~\ref{fig:weak_3_4_pt})
\end{itemize}
...
\begin{figure}
    ...
    %    \includegraphics[width=0.2\columnwidth]{fig-redraw-005.pdf}
    %\includegraphics[width=0.7\columnwidth]{fig-redraw-053.pdf}
...
\begin{itemize}
  \item Perturbation theory with a lattice regulator\aidx{lattice quantum field theory (LQFT)}\aidx{perturbation theory!lattice}\aidx{regulator!lattice} proceeds similarly to what you have done before with Feynman rules\aidx{Feynman rules}
  \begin{itemize}
  ...
  \item Loop momenta are integrated over first Brillouin zone\aidx{Brillouin zone}\aidx{Brillouin zone, first}
\begin{equation}
...
where we have assumed infinite volume.
  ...
  \item In the symmetric phase, there is only a four point interaction, but in the broken phase there is also a three point interaction (replacing one of the legs in the four point interaction with an insertion of the vev\aidx{vacuum expectation value (vev)})
...
\begin{figure}
    \centering{\input{tikz_figures/testphi}}
...
\begin{itemize}
  \item Lattice regulated integrals\aidx{lattice-regulated integrals} are quite ugly and need different techniques to dim. reg., e.g. in \(D=4\)
\end{itemize}
%
\begin{equation}
\begin{aligned}
...
\begin{itemize}
  \item Renormalisation\aidx{renormalisation}: renormalised quantities should be regulator independent
  \begin{itemize}
      \item In this theory, there are two couplings and a field renormalization\aidx{field renormalisation}
      \item The two-point vertex function\aidx{vertex function} fixes $Z$ and $m_R$ (recall that $\Gamma^R_{(n)} = Z^{n/2}\Gamma_{(n)}$)
  \end{itemize}
...
\end{equation}
    ...
    \begin{itemize}
        \item Since the one-loop diagram contributing to the two-point function\aidx{correlation function!two-point} is momentum independent, 
        \begin{equation}
            Z = 1 + O(\lambda_0^2),
        \end{equation}
...
\item Weak coupling expansion\aidx{expansion!weak-coupling} describes the theory for small $\lambda_0, \lambda_R$ and we can already make statements about the field theory in the continuum limit\aidx{continuum limit} for this region, for example computing the $\beta$-function, but let's learn more first
\end{itemize}
...
\begin{figure}
    \centering
    \redrawcompare{fig-notes-094.pdf}{fig-redraw-094.pdf}
...
\end{aligned}
\end{equation}
where $z_0$ is the single site partition function\aidx{partition function}\aidx{partition function!single-site} and $\Omega$ is the total number of sites.
...
\begin{itemize}
  \item To learn about renormalisation, need to compute the following (ie propagator and 4-point vertex\aidx{vertex function})
...
\begin{figure}
    ...
    \caption{Region of $O(n)$ phase diagram\aidx{phase diagram}\aidx{phase diagram!QCD} controlled by the hopping expansion.
    \label{fig:hoppdiag2}}
\end{figure}
...
\begin{itemize}
  \item Since \(m_{R}=\overline{m_{R}} a<1\), cutoff effects\aidx{cutoff effects}\aidx{lattice artifacts} are mild (and get smaller as $\kappa\to\kappa_c$) so at \(\kappa / \kappa_{c}=0.95\) are in scaling region\aidx{scaling region}
\item Since $\lambda_R/(16\pi^2)\lesssim 1/3$, renormalised perturbation theory\aidx{perturbation theory!renormalised} works in this regime (even though $\lambda_0\to\infty$!)
...
\begin{itemize}
  \item ``Wilson (-Kadanoff)''-like RGE\aidx{renormalisation group equations}

Wilsonian approach to RG\aidx{renormalisation group!Wilsonian (Wilson-Kadanoff)} is how does action change (infinite \# couplings) as modes are integrated out to leave low energy physics unchanged.

Consider the set of bare theories with the same renormalized $g_R$ - these define a one parameter subspace in the $m_0 a$, $\lambda_0 \sim g_0$ plane and every point on it has a different $m_R a = \frac{1}{\Lambda}$, where $\Lambda$ is the cutoff. $m_R a$ is then a dimensionless quantity character\aidx{characters (group)}izing the cutoff.

Renormalized vertex functions\aidx{vertex function} differ from the continuum limit\aidx{continuum limit} by scaling violations\aidx{scaling violations}
\begin{equation}
...
The dependence on \(m_{R} a\) comes only from scaling violations. \(\beta_{WK}(g_0)\) controls how $g_0$ changes with the cutoff.
  ...
  \item Callan-Symanzik RGE\aidx{Callan-Symanzik equation}


How does renorm coupling change as cutoff changes for fixed bare coupling\aidx{coupling!bare}?
...
%
    ...
    \item The two types of beta functions\aidx{beta function@$\beta$-function} are related
\end{itemize}
\begin{align}
...
\begin{figure}
    \centering
    \redrawcompare{fig-notes-091.pdf}{fig-redraw-091.pdf}\qquad
    \redrawcompare{fig-notes-092.pdf}{fig-redraw-092.pdf}
    \caption{Left: Potential $\beta_{CS}(g_R)$, with two IR fixed point\aidx{fixed point!infrared (IR)}\aidx{infrared fixed point}s where theory converges as cutoff $\to\infty$ and one UV fixed point\aidx{fixed point!ultraviolet (UV)} which repels as cutoff is taken away.
...
\end{figure}
\begin{itemize}
  \item In the regime of $g_R \rightarrow 0$, $\beta_{CS}$ and $\gamma_{CS}$ can be computed in renormalized perturbation theory\aidx{perturbation theory!renormalised}. Close to $a \rightarrow 0$ this can be done in the continuum theory
\end{itemize}
...
\end{itemize}
\(\Rightarrow\) Back to Lüscher-Weisz (LW)\aidx{Lüscher-Weisz analysis} results\\
\begin{figure}
    \centering
...
\begin{itemize}
    \item LW integrated RGE up to critical line\aidx{critical line} for the region where the hopping parameter\aidx{kappa@$\kappa$ (hopping parameter)} expansion\aidx{expansion!hopping} is controlled, finding no other fixed points!!
    \item LW also showed through other techniques that we won't go into that the phase diagram\aidx{phase diagram}\aidx{phase diagram!QCD} is also controlled for $\kappa > \kappa_C$ in the scaling regime\aidx{scaling region} (and numerical calculations can be performed)
...
\end{tabular}
\end{table}
In all cases except $\lambda_R = 0$, the bare coupling $\rightarrow \infty$ before the continuum limit is reached (i.e. reaches $\bar{\lambda} = 1$ for $\kappa < \kappa_C$, see Fig.~\ref{fig:On_beta_func_final}. This means that for any non-zero renormalized coupling\aidx{coupling!renormalised}, there is no continuum limit\aidx{triviality}!!
\begin{figure}
    \centering
...
\section{Blocking and fixed-point actions for scalar fields}

\todo{Add this here from the frontier topics section}