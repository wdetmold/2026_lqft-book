\section*{Exercises}
\problem
    {{\it Improved actions and reflection positivity\aidx{reflection positivity}}
    
    %Following S Sharpe 
    Consider a potential $V(\phi)=\lambda |\varphi|^4$ in 4D and construct the isotropic complex scalar field theory action with lattice spacing\aidx{lattice spacing} $a$ that has classical lattice discretisation errors of ${\cal O}(a^6)$ rather than ${\cal O}(a^4)$ using next-to- and next-to-next-to-nearest neighbour couplings. 
...
\problem
{{\textit{Free scalar field Lagrangian on a BCH lattice\aidx{lattice!body-centered hypercubic (BCH)} in $d$-dimensions.}}
...
Construct the scalar field Lagrangian on this geometry such that the correct classical continuum limit\aidx{continuum limit} is obtained.
}
...
\]
where \(\delta\) is a length scale character\aidx{characters (group)}istic of the displacement. A natural choice is the distance to a nearest neighbor,
\[
