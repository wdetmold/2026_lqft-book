...
Companion code is in \texttt{code/d4/}.
...
\item[(b)] The \emph{body-centred hypercubic} lattice in $d$ dimensions is
  ...
  which is the dual lattice $D_d^*$. Show that $D_d^*$ is similar to $D_d$
  \emph{only} for $d=4$, by comparing the lengths of its two candidate shortest
  vectors. (This is why $d=4$ is the natural home for this whole sequence;
  compare your earlier general-$d$ BCH calculation.)
...
the glue vectors $(\pm\tfrac12,\dots,\pm\tfrac12)$, with
$|x|^2 = d/4$. The lattice is similar to $D_d$ --- which has a single
shortest-vector orbit --- only if these two lengths coincide, i.e. $d/4=1$,
$d=4$. For $d<4$ the glue vectors are strictly shorter, for $d>4$ strictly
longer, and in either case $D_d^*$ has two inequivalent short shells while
$D_d$ has one. For $d=4$, $\sqrt{2}\,D_4^*$ has minimum norm $2$, covolume
$(\sqrt2)^4\cdot\tfrac12 = 2$ and kissing number $8+16=24$, matching $D_4$; the
explicit similarity is the orthogonal map $H$ of
...
two $+\tfrac14$ and two $-\tfrac14$ products), so $H^\mathsf{T}H=\mathbb{1}$ and
$H^2=\mathbb{1}$. On shell vectors, e.g.
...
$|G|^{-1}\sum_{g}\det(\mathbb{1}-tg)^{-1}$, which for $\mathrm{Aut}(D_4)$
