...
\begin{enumerate}
    \item[(a)] Show that for two observables $F$ and $G$ with support in $\Lambda_+$, reflection positivity implies that 
    $$
...
\begin{enumerate}
    ...
    \item[(d)] Discuss how this provides an upper bound on the rate of growth of the static quark potential
...
Then set F=product of 3 sides of plaquette and G=4th side. Then for reflection in plane containing G, $\Theta(G)=G^\dagger$ so $G\Theta(G)=1$ and $F\Theta(F)=W_{2,1}$ is 2x1 loop
...
\begin{enumerate}
    \item Show that the Wilson gauge action 
    $$
    ...
    $$
    Show that the correct continuum limit is obtained. Find $\gamma$ and $\hat\beta$ such that the leading discretisation artifacts are ${\cal O}(a^4)$ and find the relation between $\hat\beta$ and $\beta$ in the unimproved action.
