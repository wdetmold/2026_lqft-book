...
\end{figure}
\begin{itemize}
  \item In many situations, physical quantities are accessed from hadronic matrix elements\aidx{hadronic matrix element} of external-to-QCD currents/operators
  \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item The expectation values of fields: $\langle \Omega | \bar{\psi}(x) \psi(x) | \Omega \rangle$ = chiral condensate\aidx{chiral condensate}
        \item The vacuum polarisation\aidx{vacuum polarisation} (relevant for $(g-2)_\mu$):
    \end{itemize}
\end{itemize}
\begin{equation}
...
\begin{itemize}
    \item For quantities such as meson decay constants\aidx{decay constant}, meson and baryon distribution amplitudes\aidx{distribution amplitude}, proton decay matrix elements\aidx{proton decay}, the relevant matrix element is of the form
\end{itemize}
\begin{equation}
    \langle H(\vec{p}) | \tilde{{\cal O}}(\vec{p}) | \Omega \rangle
\end{equation}
which can be accessed from two point functions\aidx{correlation function!two-point}
\begin{equation}
...
\begin{itemize}
    \item Common examples of hadronic matrix elements\aidx{hadronic matrix element} accessible from three point functions are $\langle H_1 | {\cal O} | H_1 \rangle, \langle H_1 | {\cal O} | H_2 \rangle$ (i.e. including transitions)
    \begin{itemize}
        \item In terms of electroweak hadron structure ${\cal O} = J^\mu_\text{EM}, J^\mu_\text{EW}$ are most relevant and determine the form factors\aidx{form factor} describing hadron EW structure (e.g. $\langle \pi(\vec{p})|\tilde{J}_\mu(\vec{q})|\pi(\vec{p} - \vec{q})\rangle \sim F_\pi(q^2)$)
        \item In quark flavour physics, a common example is the flavour changing neutral current\aidx{flavour-changing neutral current} decay $B \to K$. Such decays are mediated by physics at the electroweak scale and above and from the point of view of QCD enter through an effective low energy Hamiltonian\aidx{effective weak Hamiltonian}
    \end{itemize}
\end{itemize}
\begin{equation}
    H_\text{weak} = \sum_i C_i(\mu){\cal O}_i(\mu),
\end{equation}
where the $C_i$ are Wilson coefficients\aidx{Wilson coefficients} which depend on high scale physics and where for $\Delta S = \Delta B = 1$ processes the lowest dimension operators are $4q$ operators
\begin{equation}
...
\begin{itemize}
    \item In such cases, we look at three-point functions\aidx{correlation function!three-point} to extract these matrix elements:
\end{itemize}
...
\end{align}
where on the last line the two matrix elements involving $\chi$ are overlap factors\aidx{overlap factor} and the other matrix element is the desired matrix element.
\begin{itemize}
    \item The energies and overlap factors can be determined from two-point correlation functions\aidx{correlation function}\aidx{correlation function!two-point} not involving the operator ${\cal O}$ and thus the above gives access to the matrix element
...
\begin{itemize}
    ...
    \item For studies of parton distribution functions\aidx{parton distribution functions}, ${\cal O}$ can be a nonlocal\aidx{locality (of lattice Dirac operators)} bilinear operator ${\cal O} = \bar{\psi}(x) W_C(x,y) \psi(y)$, where $W_C$ is a Wilson line\aidx{Wilson line}.
\end{itemize}

\section{Four- and higher-point correlation functions}

\begin{itemize}
    \item More complicated correlation functions\aidx{correlation function} can be used to access additional physics such as second order electroweak processes\aidx{second-order weak processes}
    \begin{itemize}
        \item $K \to \pi \nu \bar{\nu}$
        \item $p p \to n n e e \nu \nu$ or $p p \to n n e e$
        \item the hadronic tensor\aidx{hadronic tensor} $T^{\mu \nu} \sim \langle p | J^\mu J^\nu | p \rangle$ relevant in DIS
        \item ...
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item This is the case even for operators that are renormalisation scale independent since the lattice discretisation can introduce a finite shift (e.g. axial current\aidx{axial current}): 
        \begin{equation}
            {\cal O}_{1,\text{bare}} = Z_1^{-1}\mathcal{O}_{1, \text{ren}}
        \end{equation}
        \item Since the lattice regulator\aidx{lattice quantum field theory (LQFT)}\aidx{regulator!lattice} introduces a dimensionful scale, it allows for mixing of operators of different dimensions\aidx{operator mixing} 
        \begin{equation}
    ...
    \item Operator renormalisation\aidx{operator renormalisation} can be performed in multiple ways
    \begin{itemize}
        \item Perturbatively using analytic/numerical calculation of Feynman diagrams\aidx{renormalisation!perturbative} derived from the lattice action and lattice regulated intergrals - typically the same loop calculations must also be done in DR-$\overline{\text{MS}}$ to connect to $\overline{\text{MS}}$ renormalisation scheme: 
        \begin{equation}
            Z \sim 1 - (\text{loops})_\text{latt} + (\text{loops})_\text{DR}.
        \end{equation}
        Lattice PT is notoriously complicated and poorly convergent.
        \item Non-perturbatively using the Rome-Southampton method (RI-MOM). This makes use of an intermediate renormalisation scheme that can be defined in a regulator independent way such as a momentum subtraction scheme\aidx{renormalisation!RI-MOM}\
        \item Non-perturbatively using the Schr\"odinger functional step-scaling\aidx{step scaling} method which uses the lattice size $L \sim \mu^{-1}$ as the renormalisation scale and allows running of operators to be probed over large scale differences by not requiring all scales to be represented in a single lattice geometry (see later)
    \end{itemize}
...
\end{equation}
where $\Gamma_A$ is the vertex function\aidx{vertex function} $\Gamma_A(p,q) = S^{-1}(p)\Lambda_A(p,q)S^{-1}(p+q)$. At tree level,
\begin{equation}
...
\begin{itemize}
    \item For LQCD\aidx{lattice QCD (LQCD)} we must fix the gauge for quark momenta to be defined: Landau gauge\aidx{gauge fixing!Landau} $(\partial_\mu A_\mu = 0)$. It is necessary to check whether gauge dependence of the results is small.
    \item The symmetric momentum point\aidx{renormalisation!RI-SMOM} $p_1^2 = p_2^2 = (p_1 - p_2)^2 = \mu^2$ with $\Lambda_{\text{QCD}} \ll \mu^2 \ll 1/a^2$ is chosen such that all momenta are perturbative and discretisation effects are small (ideally at least). For other momenta choices care must be taken due to non-perturbative contributions.
    \item Quark wavefunction renormalisation\aidx{quark wavefunction renormalisation} from
\end{itemize}
\begin{equation}
...
\begin{itemize}
    \item Since the MOM schemes are regulator independent, the continuum limit\aidx{continuum limit} can be taken at this point: $Z_A$ has lattice artifacts\aidx{lattice artifacts} that must be removed
...
\begin{itemize}
    ...
    \item Nuclear matrix elements\aidx{matrix element!nuclear} are a frontier topic
    \item Second order weak processes\aidx{second-order weak processes} are another frontier
    \item See FLAG\aidx{FLAG} for up-to-date values
\end{itemize}
