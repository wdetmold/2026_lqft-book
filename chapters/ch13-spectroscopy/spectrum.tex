...
\begin{itemize}
    \item A central property of QCD is the spectrum of stable particles, defined by the lowest energy state of each set of unique quantum numbers that are preserved by QCD:
\end{itemize}
\begin{equation}
    \pi, K, D, B, D_s, B_s, B_c, p, n, \Lambda, \Sigma, \ldots, d, {}^3\text{He}, {}^3_\Lambda\text{He}, \ldots, T_{cc}^+(cc \bar{u}\bar{d})
\end{equation}
\begin{itemize}
    \item For resonances (``particles'' that are unstable) in Minkowski space, it is possible to extract information about them from the excited states at finite volume
\end{itemize}

\section{Interpolating operators}

\begin{itemize}
    \item The simplest way to determine such energies is through calculation of a two-point correlation function involving ``interpolating operators'' that create and destroy states with the appropriate quantum numbers from the vacuum state
    \begin{itemize}
...
\end{itemize}
First, we consider the (negative) charged pion:
\begin{equation}
...
\end{itemize}
And finally the deuteron ($B=2, J=1, I=0$):
\begin{align}
...
\begin{itemize}
    ...
    \item One simple smearing (wall smearing) that can be implemented cheaply by first gauge fixing to Coulomb gauge is to project quarks to a fixed momentum
\end{itemize}
\begin{equation}
...
\begin{itemize}
    \item An example of a gauge covariant smearing is Jacobi smearing
\end{itemize}
\begin{equation}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Glueballs may have enterpolating operators constructed entirely from gauge invariant combinations of links. They also have overlap with isoscalar quark interpolators
        \item Closed paths of links can be added to any operator and may increase the overlap factors for particular states
    \end{itemize}
\end{itemize}
\section{Two point correlation functions}
\begin{itemize}
    \item Given interpolating operators, the bilocal two-point function (pion case)
\end{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{fig-notes-082.png}
...
\begin{itemize}
    \item For a proton, need a bit more care so there are multiple possible Wick contractions
\end{itemize}
\begin{align}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{fig-notes-083.png}
...
\begin{enumerate}
    ...
    \end{equation}
    \item Solve for the smeared-source, unsmeared-sink  (LS) propagator using CG etc
    \begin{equation}
        D {}^{LS}S = {}^S \mathcal{S}
    \end{equation}
    ...
    \item Contract propagators together to form appropriate correlation functions
\end{enumerate}

\section{Flavour singlet states}

\begin{itemize}
  \item A correlation function for a flavour singlet set of quantum numbers presents an additional complication. Consider
\end{itemize}
\begin{align}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{fig-notes-084.png}
...
\end{figure}
\begin{itemize}
    \item The first term is just like the $\pi^-$ case above, but the second requires knowledge of the quark propagator from every point to itself which would require solution of $V$ linear systems and is infeasible
    ...
    \begin{itemize}
        \item Hutchinson trace estimation: given a set of $N_H$ random $\mathbb{Z}_2$ vectors $\{z_{(i)}\}$ distributed such that for each entry $z_{(i)j}$, $j \in \{ 1, \ldots, N_d N_c V \}$, $\text{Pr}(z_j = \pm 1) = \frac{1}{2}$ then
        \begin{equation}
        ...
        \end{equation}
        \item For efficiency this is typically combined with ``dilution'' in which for each $\mathbb{Z}_2$ vector, non-zero values are only allowed in a subset of elements
        \item Other variants of the noise (i.e. not $\mathbb{Z}_2$) are also used
        \item The current state-of-the-art is known as hierarchical probing but the details are omitted
    \end{itemize}
    \item Whatever method is used, the numerical cost is large compared to that required for flavour non-singlets
    \item Access to flavour singlet spectrum is important for glueball spectroscopy in QCD since gluonic interpolating operators mix with flavour singlet quark operators and in understanding the $\eta'$ mass and the Witten-Veneziano formula
    \begin{equation}
        m_{\eta'}^2(m_q = 0) = \frac{4 N_f}{f_0^2}\chi_\infty,
    \end{equation}
    where $f_0$ is the singlet decay constant and $\chi_\infty$ is the topological susceptibility in YM $\chi_\infty \sim \langle Q^2 \rangle$.
\end{itemize}

\section{The transfer operator}
\begin{itemize}
    \item The transfer operator $(\hat{T} = e^{- a \hat{H}})$ governs the Euclidean time evolution of correlation functions and is applicable to propagate states between timeslices of the lattice geometry provided no operators are on these timeslices
    \item A given two-point function (with $\tilde \chi_a(\vec{p}, t) = \sum_{\vec{x}} e^{i \vec{p} \cdot \vec{x}} \chi_a(\vec{x}, t)$):
\end{itemize}
\begin{align}
...
Taking the large $T$ limit forces $|m \rangle \rightarrow |\Omega \rangle$, the vacuum state, with $\vec{p}_m = \vec{0}$ and the partition function $Z \rightarrow 1$. The sum $\sum_x \rightarrow \delta(\vec{p} - \vec{p}_n)$ since $\vec{p}_m = \vec{0}$ and, choosing $\chi_a = \chi_b = \chi$ for simplicity, this yields
\begin{align}
...
That is, up to the effects of the finite temporal extent (which can also be included), the existence of a positive definite transfer matrix guarantees that correlation functions are weighted sums of exponentials in $t$ and if the source and sink are identical they are sums with positive coefficients
...
\begin{itemize}
    ...
    \item More sophisticated fitting strategies (and precise data) allow excited state energies $E_n(\vec{p})$, $n> 0$, to be determined but it becomes increasingly challenging and is formally an ill-posed problem as the number of unknown energies is larger than the extent of any time series that can be calculated
...
\begin{itemize}
    \item Given a set of quantum numbers, many interpolating operators can be constructed. It is possible to make use of a set of operators $\{\chi_1, \ldots, \chi_p \}$ to form a matrix of correlation functions
\end{itemize}
\begin{equation}
...
\begin{itemize}
    ...
    \item A variational approach can be used to construct the linear combination of the $\chi_i$'s that best interpolates to a particular state
    \item This can be phrased as a generalised eigenvalue problem
\end{itemize}
...
\begin{itemize}
    ...
    \item Matrices of correlation functions can also be analysed using fits to constrained sums of exponentials
    ...
    \end{itemize}
    \item Linear combinations of interpolating operators can also be optimised for other goals such as reducing the sample variance (see later)
\end{itemize}

\section{Rayleigh-Ritz methods}

\begin{itemize}
  \item A single correlation function corresponds to to matrix element of the transfer matrix. Here we assume a symmetric correlation function
...
where $|\psi\rangle=\psi(0)|\Omega\rangle$
  \item We can use the correlation function to generate a Kryloy space
...
Now, \(\left|k_{i}\right\rangle\) are not eigenstates of \(T\), so \(G_{i j} \neq \delta_{i j}\)
  ...
  \item Introduce Cholesky decomposition \(G=L^{\dagger} L\) and define
\begin{equation}
...
where $T^{(m)}$ is the $m^{\text{th}}$ iteration approximation to $\hat T$
  ...
  \item Extract by solving the Generalised Eigenvalue Problem (GEVP)

\begin{equation}
M \vec{g}_{k}=\lambda_{k} G \vec{g}_{k}
\end{equation}

  \item Can be extended to asymmetric correlation functions and to matrices of correlation function [see 2503.17357]
\end{itemize}


\section{Statistical behaviour of correlation functions}
\begin{itemize}
    \item Since the Monte Carlo approach provides stochastic estimators of correlation functions it is important to understand their statistical behaviour
...
\end{equation}
where $\hat{C}(t)$ is the exact correlation function, and the sample variance is given by
\begin{equation}
    \text{var}(C(t)) = \langle | \mathcal{C}(t)|^2 \rangle - |\langle \mathcal{C}(t) \rangle | ^2
\end{equation}
\begin{itemize}
    \item Since QCD is local, far separated regions of spacetime larger than any physical length scale are effectively decorrelated, volume averaging (in which correlation functions are computed in distinct regions) can be used as a means to construct an improved estimator. In the $V \rightarrow \infty$ limit, a single ``master field'' configuration is all that is required but only in the limit - away from $V = \infty$, you must sample sufficiently
...
\begin{itemize}
    ...
    \item There is a close but not fully quantified connection between this signal-to-noise problem at the sign problem at finite density
\end{itemize}


\section{The state -of - the -art}
\begin{itemize}
    \item The low energy stable states in the hadron spectrum are reproduced to few percent accuracy $(\pi, K, p, \Lambda, \Lambda_b, \ldots)$ - THIS IS A BIG DEAL! IT IS NECESSARY EVIDENCE THAT QCD DESCRIBES THE STRONG INTERACTIONS
    \item In the bottom and charm sectors, a number of LQCD predictions have been verified
    ...
    \item The state of the art using variational methods extracts $\mathcal{O}(20)$ states of the given system 
\end{itemize}
