...
\begin{itemize}
\item We now have all the tools to undertake many different LQFT calculations. Here we will walk through the steps required to perform a lattice QCD calculation of a physical quantity $\mathcal{P}$
\end{itemize}
\begin{enumerate}
    ...
    \begin{itemize}
        \item e.g. if chiral symmetry prevents operator mixing, a chiral fermion discretisation would be useful
        \item the computational costs vary significantly with this choice (otherwise every calculation would use overlap fermions)
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item In order to tune the quark masses to their physical values and determine the lattice spacing in physical units, the same number of experimentally determined quantities must be used (and computed): often the masses of various mesons and baryons are chosen along with the static quark potential or the electric field energy extracted from ``Wilson flow''
    ...
    \begin{enumerate}
        ...
        \item calculating fermion matrix inverses for construction of correlation functions
...
\end{enumerate}
\begin{figure}
\includegraphics[max width=0.8\textwidth, center]{fig-notes-022.jpg}
...
\begin{itemize}
    ...
    \end{itemize}
    \item Since these calculations require a lot of computing, the gauge configurations (and also in some cases the quark propagators) are saved and can be reused for other physics goals - this may constrain the parameter sets $\mathcal{S}_i$ that are used
\end{itemize}

\section{State-of-the-art}

\begin{itemize}
    \item LQCD calculations are often carried out by large collaborations, with state of the art parameter sets for staggered/Wilson fermions using quark masses including the physical values and lattice geometries up to $128^3\times256$ with lattice spacings $0.04 \text{ fm} < a < 0.15 \text{ fm}$ typical. For chiral fermions the lattice geometries are a factor of $\sim 2$ smaller in each dimension. Typically $\mathcal{O}(100)-\mathcal{O}(1000)$ somewhat independent configurations are produced
    ...
    \item State-of-the-art calculations for some quantities include QED as well as QCD [since QED is not asymptotically free, it has no continuum limit and should be thought of as an effective theory]
    \item State of the art calculations use tailored variants of HMC for gauge field generation and typically use multigrid fermion inverters. These are implemented in community libraries
...
\begin{itemize}
    \item QCD has a rich structure at nonzero temperature and/or density, with a conjectured phase diagram shown in Fig.~\ref{fig:phase}
    ...
    \item As in statistical mechanics, nonzero temperature and chemical potentials can be introduced into the grand canonical partition function as
\begin{figure}
\includegraphics[max width=0.8\textwidth, center]{fig-notes-023.jpg}
\caption{Conjectured QCD phase diagram.
\label{fig:phase}
}
\end{figure}
\begin{equation}
...
\item In terms of a path integral,
\begin{equation}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item entropy density $s = (\varepsilon + P)/T$ (NB: the interaction measure $I = \varepsilon - 3 P$ is useful)
        \item speed of sound $c_s^2 = \frac{\partial P}{\partial \varepsilon}$
    \end{itemize}
\end{itemize}

\begin{itemize}
    \item These can be computed perturbatively at asymptotically high energy and modeled at low energy using the hadron resonance gas model
    \item To compute thermodynamic quantities in lattice QCD one can consider an anisotropic lattice geometry $L^3 \times L_4$ with $L = a N_s, L_4 = a_t N_t = T^{-1}$ with $\xi \equiv a_s/a_t$ the anisotropy and note that
\begin{align}
...
which is equivalent to integration over $T$, and then $\Delta \varepsilon = \Delta I + 3 \Delta P$.
    \item These quantities characterise the QCD equation of state and can be compared to low and high temperature expectations.
    \item From extensive lattice calculations, the picture is as demonstrated in Fig.~\ref{fig:thermo} with $N_f = 3$.
    \item Since the addition of chemical potential makes the Euclidean action complex, $\exp[-S]$ is no longer interpretable as a probability measure and so importance sampling is inapplicable - a sign/phase problem
    \begin{itemize}
        \item There are exceptions to this such as if gauge group is $SU(2)$ or $G_2$ which have only (pseudo-)real representations or for $SU(3)$ with $\mu_u = - \mu_d$ (isospin chemical potential)
    \end{itemize}
    \item It is useful to expand thermodynamic quantities about zero $\mu_i$, e.g.
\begin{figure}
\includegraphics[max width=0.8\textwidth, center]{fig-notes-024.jpg}
...
\begin{itemize}
  \item At zero chemical potential, QCD is confining at low temperature but deconfines at high temperature (asymptotic freedom guarantees this) and a crucial question is at what temperature this happens and what is the nature of this transition
  \item In Yang Mills theory, an order parameter for confinement is $\langle P \rangle$ where the Polyakov loop
\begin{equation}
...
To see this, note that the correlator between two Polyakov loops separated by a spatial distance $R$ is related to the potential between an infinitely massive quark and antiquark
\begin{equation}
...
Since $\lim_{R\to\infty}\langle P(0)P(\vec{R})^\dagger \rangle = \langle P(0)\rangle \langle P(R)^\dagger\rangle = |\langle P \rangle |^2$, in order for $F_{\bar{Q}Q}(R) \sim \sigma R$, corresponding to linear confinement, we must have $\langle P \rangle = 0$.
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{fig-notes-025.jpg}
    \caption{A schematic showing the behavior of the Polyakov loop as a function of temperature.}
    \label{fig:polyakov}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{fig-notes-026.jpg}
...
\end{figure}
    ...
    \item The deconfining transition corresponds to the spontaneous breaking of the $Z_{N_c}\subset SU(N_c)$ center symmetry
\begin{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{fig-notes-080.png}
...
\end{figure}
    ...
    \item At nonzero temperature as one moves into the quark gluon plasma phase where interactions become weak due to asymptotic freedom, spontaneous breaking of chiral symmetry must also disappear (it does not occur at any order in perturbation theory)
    \item An order parameter for $\chi$SB is the chiral condensate,
\begin{equation}
...
\begin{figure}
\includegraphics[max width=0.6\textwidth, center]{fig-notes-029.jpg}
\caption{Schematic of the chiral condensate as a function of temperature.\label{fig:chi_cond}
}
\end{figure}
\begin{figure}

\includegraphics[max width=0.6\textwidth, center]{fig-notes-027.jpg}
\caption{Schematic of the susceptibility of the chiral condensate as a function of temperature.\label{fig:chi_sus}}
\end{figure}
\begin{itemize}
    \item Careful analysis shows that in $2 + 1$ flavour QCD the chiral crossover temperature for physical quark masses is $T_c = 155(10) \text{ MeV}$.
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{fig-notes-028.jpg}
...
\begin{itemize}
    \item $U_A(1)$ symmetry is anomalous and is broken in QCD at zero temperature by topologically non-trivial configurations of the gluon field. It is expected to be restored at high temperature as the importance of instantons decreases due to screening of colour charge.
        \item There is no order parameter for $U_A(1)$ restoration but under a $U_A(1)$ transformation $\vec{\pi} = \bar{\psi} \gamma_5 \vec{\tau} \psi \rightarrow \bar{\psi} \vec{\tau} \psi = a_1$, so if it is restored the pion and $a_1$ mesons should be degenerate. The quantity
\begin{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{fig-notes-081.png}
    \caption{The quantity characterising the nondegeneracy between the pion and the $a_1$ as a function of temperature, showing axial symmetry restoration above $T_C$.
        \label{fig:asr}}
\end{figure}
...
\begin{itemize}
    \item A key goal of QCD thermodynamics is to determine the position and nature of the critical point at $\mu \neq 0$ at which the $1$st order phase transition starts
    \item Nonzero $\mu_q$ is problematic and only limited information can be obtained. Methods include:
    \begin{itemize}
        \item Use imaginary chemical potential and analytically continue to real $\mu$
        \item Taylor expand about $\mu = 0$ using susceptibilities and estimate radius of convergence using $\chi^n/\chi^{n-2}$ since the critical point is a singularity
        \item reweighting: consider a partition function that treats the chemical potential as part of the observable
    \end{itemize}
\end{itemize}
\begin{align}
...
\begin{itemize}
    \item In order to characterise the properties of the QGP, the way in which the medium responds to excitations ($=$ transport properties) can be investigated by looking at correlation functions between local operators
...
\end{align}
defines the spectral function $\rho_{VV}$
Note that this requires undoing the Legendre transformation to get $\rho$. In terms of this, the electrical conductivity is given by
\begin{equation}
...
\end{align}
the sheer and bulk viscosities are then given by
\begin{align}
