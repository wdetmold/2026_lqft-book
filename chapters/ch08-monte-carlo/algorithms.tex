...
\section{The computational challenges}
...
\begin{itemize}
  \item The basic numerical task in LQFT\aidx{lattice quantum field theory (LQFT)} is evaluation of correlation functions\aidx{correlation function} which corresponds to very high dimensional integrals with specific structure
\end{itemize}
...
\begin{itemize}
  \item For fermionic theories, Grassman integration is done analytically and requires determinants and inverses of the Dirac operator\aidx{Dirac operator}
\end{itemize}
...
\begin{itemize}
    \item A Markov chain is a stochastic sequence of states/configurations of the degrees of freedom of a system defined by a transition probability\aidx{Markov chain!transition probability} $T(s,s')$ from a state $s'$ to a state $s$ in which the transition only depends on $s$ and $s'$ and not on other states in the chain:
\end{itemize}
\begin{equation}
...
\begin{itemize}
  ...
  \item To generate these samples, a Markov process\aidx{Markov chain} needs to have two properties
\end{itemize}
\begin{enumerate}
  \item The process must have \(P_{\text{eq}}\) as a fixed point\aidx{infrared fixed point}. That is
\end{enumerate}
...
\begin{itemize}
  \item The balance condition is guaranteed by the detailed balance condition\aidx{detailed balance} (easier to impose)
\end{itemize}
...
\begin{enumerate}
  \setcounter{enumi}{1}
  \item Since the integral that is being sampled is over all values of the fields the Markov process should be ergodic\aidx{Markov chain!ergodicity}: it is possible to get to every state. Thus $0 < T(s, s')^n < 1 \quad \forall s,s'$ and some $n$. It does not need to be a single step to get to every state.
\end{enumerate}
\begin{itemize}
...
\begin{itemize}
  ...
  \item For an individual Markov process after thermalisation\aidx{Markov chain!thermalisation}, sequences of samples will be representative of the target distribution. However these sequences will have autocorrelation (in most cases): nearby samples in the sequence will be closely related
  \end{itemize}
\end{itemize}

For a sequence of states \(\left\{s_{1}, \ldots  s_{N}\right\}\) and a quantity \(A(s)\) the integrated autocorrelation time\aidx{autocorrelation!integrated time} is approximated by
...
\end{equation}
with $A_i \equiv A(s_i)$ and $\langle A \rangle = \frac{1}{N}\sum_i A_i$. NB: this is observable dependent, we can define the exponential autocorrelation time\aidx{autocorrelation} as $\tau_\text{exp} = \operatorname{sup}_A \tau_{\text{int},A}$.
...
\end{equation}
and thus the effective number of independent samples\aidx{independent samples, effective number of} for quantity $A$ is $N_\text{indep} = N/(2 \tau_{\text{int},A})$.
\begin{itemize}
...
\begin{itemize}
    \item Since $P_\text{eq} = \frac{1}{Z}e^{-S_\text{eff}(s)}$ is known up to the normalisation, choosing the transition probability\aidx{Markov chain!transition probability}
\end{itemize}
\begin{equation}
    T_{\text{HB}}(s', s) = \frac{1}{Z} e^{-S_\text{eff}(s')}
\end{equation}

satisfies detailed balance\aidx{detailed balance} trivially:
\begin{align}
...
\begin{itemize}
  \item However given that $s$ represents an entire field configuration over continuous degrees of freedom in our context there is no way to efficiently use such a global heatbath\aidx{algorithm!heatbath} efficiently
\end{itemize}

\section{Metropolis Algorithm}
\begin{itemize}
    \item An algorithm satisfying detailed balance\aidx{detailed balance} can be constructed as follows from an arbitrary initial state $s$
\end{itemize}
\begin{enumerate}
    \item Choose an updated state $s'$ according to an efficiently implementable transition probability\aidx{Markov chain!transition probability} $T_0(s', s)$
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item For theories involving fermions $S_\text{eff}[\phi] \supset \operatorname{tr}\ln (D[\phi])$ is very nonlocal\aidx{locality (of lattice Dirac operators)} and so updating a single d.o.f. requires computing the entire $S_\text{eff}$, so the cost of the algorithm is $\mathcal{O}(V^2)$ - this is impractical for e.g. QCD
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Thus scalar ``pseudofermion\aidx{pseudofermions}'' fields can be used to represent the fermion determinant\aidx{fermion determinant}, so for fermions + gauge fields
\end{itemize}
\begin{align}
...
\begin{enumerate}
    ...
    \end{equation}
    \item It also holds for a single flavour for a Dirac operator\aidx{Dirac operator} obeying the GW relation\aidx{Ginsparg-Wilson relation} since eigenvalues come in complex conjugate pairs (also staggered fermions\aidx{fermions!staggered})
    \item It does not hold for a single flavour of Wilson fermions\aidx{fermions!Wilson}, since $\det D_W \in \mathbb{R}$ but not guaranteed to be positive. However for large enough mass and lattice volume, negative eigenvalues are exponentially suppressed and it is typical to use for e.g. strange quarks $\det D_s \approx [\det D_s^\dagger D_s]^{1/2}$
    \item In the presence of fermion chemical potential\aidx{chemical potential} or the QCD $\theta$-term the determinant is nonpositive and importance sampling fails\aidx{importance sampling}
\end{enumerate}

\begin{itemize}
  \item Since \(D^{-1}[U]\) is very non-local\aidx{locality (of lattice Dirac operators)}, the Metropolis algorithm\aidx{algorithm!Metropolis} will be very inefficient
\end{itemize}


\section{Hybrid Monte-Carlo Algorithm}
\begin{itemize}
    \item HMC introduces auxiliary canonical momenta\aidx{algorithm!hybrid Monte Carlo} conjugate to the field degrees of freedom and uses Hamilton's equations\aidx{molecular dynamics} to evolve the system according to molecular dynamics
    \begin{itemize}
    ...
    \item HMC uses these equations to propose new ``$q$''s that will have constant action and the update probability satisfies ergodicity\aidx{Markov chain!ergodicity} and detailed balance\aidx{detailed balance} provided the integration along the HMC trajectory is symplectic (area preserving) and reversible
    \item Specifying to a theory with $N_f = 2$ fermions and gauge fields, we can write a modified partition function\aidx{partition function} (just multiplying by a constant)
\end{itemize}
\begin{equation}
...
\end{equation}
where the $P_\mu(n) \in g$ are elements of the Lie algebra $g$ and are conjugate to $Q_\mu(n)$ where $U_\mu(n) = \exp(i Q_\mu(n))$; note that the pseudofermion\aidx{pseudofermions} fields have spin and colour components.

\begin{itemize}
    \item Since correlation functions\aidx{correlation function} will never contain $P_\mu$'s they are unchanged
...
\begin{enumerate}
    ...
    \end{equation}
    using a symplectic, reversible integrator (e.g. leapfrog integrator\aidx{leapfrog integrator}). This deterministically defines $T_{MD}(\{P',Q'\},\{P,q\})$.
...
\begin{itemize}
  \item The transition probability\aidx{Markov chain!transition probability}
\end{itemize}
...
\begin{itemize}
    \item A symplectic integrator such as leapfrog\aidx{symplectic integrator} is reversible and area preserving as required for Hamiltonian system. For each integration step of length $\varepsilon$,
    \begin{enumerate}
...
\begin{itemize}
    \item In calculating correlation functions\aidx{correlation function} with fermion fields after integrating out the fermion fields we are left with products of $D^{-1}[U]$ acting on e.g. a spin-colour vector $\rho$ which can be computed by solving
\begin{equation}
...
\begin{itemize}
    ...
    \item Iterative methods\aidx{linear solver}  construct successively improving approximations to $x$: $x_1 \rightarrow x_2 \rightarrow \ldots \rightarrow x_N$ with $\|x_{i+1} - \hat{x} \| < \|x_{i} - \hat{x} \|$ where $\hat{x}$ is the exact solution
    \item Krylov solvers\aidx{linear solver!Krylov} are a class of iterative methods that work well for the sparse systems under consideration and scale only as $\mathcal{O}(N)$
    \item The conjugate gradient algorithm (CG)\aidx{algorithm!conjugate gradient} works well for Hermitian systems (NB: $D$ is not Hermitian but $D^{-1} = D^\dagger (D D^\dagger)^{-1}$, so it suffices to work with $A = D D^\dagger$ which is Hermitian)
    \begin{itemize}
...
\begin{itemize}
    ...
    \item Typically the number of iterations required for convergence to a particular precision is set by the condition number\aidx{condition number} of the matrix $A$
\end{itemize}
\begin{equation}
...
\begin{itemize}
    ...
    \item Other variants are appropriate for non-Hermitian cases and may have faster convergence (BiCG-stab\aidx{linear solver!BiCGstab})
    \item Matrices can be preconditioned in many ways (even-odd, ILU, SSGR, Schwartz, Multigrid\aidx{preconditioning}\aidx{preconditioning!multigrid}) to accelerate convergence
    \item The low eigenvalues of the matrix\aidx{deflation} can be ``deflated'' (removed), also accelerating convergence
\end{itemize}

\section{Other algorithmic tasks}
\begin{itemize}
    \item Overlap fermions\aidx{fermions!overlap} require the matrix sign function which is most optimally approximated using Zolotarev rational functions\aidx{matrix sign function}
