...
\item In the QM case, the discretisation of time was regularly spaced: length $t$ divided into $N$ segments for length $\epsilon=t/N$ and the forward finite difference $\dot x=\frac{1}{\epsilon}(x_{j+1}-x_j)$ arose naturally.
\begin{itemize}
...
\item Regular lattices: consider a linearly independent set of vectors $\{B_i, i\in\{1,\ldots,D\}\}\in \mathbb{R}^m$. A $D$-dimensional regular lattice can be defined as
\begin{equation}
...
\begin{table}
    \centering
    \begin{tabular}{lccc}\
        Cartan-Dynkin root diagram &  $D=2$& $D=3$ & $D=4$ \\ \hline
        ...
        &  square ($D_2\simeq Z_2$)& face-centered cubic ($D_3\simeq A_3$) & body-centered hypercubic \\
        $E_D$ for $D\in\{6,7,8\}$ &  &  & \\
...
\item Root lattices retain subsets of Poincare symmetries

\item in $D=4$, the BCH lattice is the most symmetric

\item Easy to consider anisotropic lattices (eg $Z_1\times Z_3$ with the constant $a$ different)

\end{itemize}

\item Irregular discrete geometries: there is no reason discrete geometry has to be regular. Can consider random lattices (not actually lattices by the above definition)

\begin{itemize}
    \item Consider a randomly distributed set of points in a volume $V$. 
    \item Build simplices by joining points as vertices with edges (a $D$-simplex has $D+1$ vertices; triangle in $D=2$, tetrahedron in $D=3$ etc)
    ...
    \item Algorithms exist for this (eg Voroni tessellation, Delauney triangulation and Random Geometric Graphs)
...
\item Random lattices appear to spontaneously break chiral symmetry (not good)

\item Simplex structure has interesting connections to finite-element methods used in other scientific computing settings

\item In contrast to CM physics, low-energy physical results should be seen to be independent of the exact lattice geometry prescription -- any choice is allowed but some (such as hypercubic lattices) are more practical. Using multiple discrete geometries is a way to test universality of LQFT prescriptions

\item A LQFT is defined by the sequence of lattices as the continuum limit is taken

\item Does a limit of lattices recover continuous space-time? This is mathematically unclear and is related to the failure of the Connes embedding conjecture
